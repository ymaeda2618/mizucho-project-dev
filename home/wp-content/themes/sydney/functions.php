 ...
 * php
 */
function original_php_form_show($params = array())
{
	...
	ob_start();
	include(STYLESHEETPATH . "/original/php/$file.php");

	return ob_get_clean();
}
add_shortcode('original_php_show', 'original_php_form_show');

/**
 * css
 */
function original_css_form_show($params = array())
{
	extract(shortcode_atts(array(
		'file' => 'default'
	), $params));
	...
	ob_start();
	include(STYLESHEETPATH . "/original/css/$file.css");

	return ob_get_clean();
}
add_shortcode('original_css_show', 'original_css_form_show');
...
function twpp_enqueue_styles() {
	wp_enqueue_style( 
	  'common-style',
	  "https://mizucho.com/home/wp-content/themes/sydney/original/css/common.css"
	);
  }
